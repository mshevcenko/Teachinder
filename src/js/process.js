...
function formatUser(user) {
  ...
  const formattedUser = {
    ...
    age: getUserField('age') || getUserField('dob', 'age') || 25,
...
function sortUsers(users, param, descending = false) {
  ...
  } else if (param === 'country' || param === 'full_name' || param === 'course' || param === 'gender') {
    compareFunc = (first, second) => first[param].localeCompare(second[param]);
  } else {
    return sortedUsers;
  }
  sortedUsers.sort(compareFunc);
  if (descending) sortedUsers.reverse();
  return sortedUsers;
}
...
function searchUsersByName(users, name) {
  ...
  return users.filter((user) => user.full_name && user.full_name.toLocaleLowerCase().includes(name));
}
...
function searchUsersByNote(users, note) {
  return users.filter((user) => user.note && user.note.toLocaleLowerCase().includes(note));
}
